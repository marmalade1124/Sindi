 ...
 please insert the parameter as follow : myExamplePage/132621766841117';
    ...
    public function collectData()
    {
        ...
        if ($this->getInput('u') !== null) {
            $username_input = $this->getInput('u');
            
            if (preg_match('/^\d+$/', $username_input)) {
                 $pageInfo = [
                     'userId' => $username_input,
                     'username' => $username_input
                 ];
            } else {
                $page = 'https://touch.facebook.com/' . $username_input;
...
EOD
        ...
        foreach ($html->find('article') as $content) {
            ...
            $content = preg_replace(
                '/ΓÇª (<span>|)<a href="https:\/\/www\.facebook\.com\/story\.php\?story_fbid=.*?<\/a>/m',
                '',
                $content,
                1
            );
